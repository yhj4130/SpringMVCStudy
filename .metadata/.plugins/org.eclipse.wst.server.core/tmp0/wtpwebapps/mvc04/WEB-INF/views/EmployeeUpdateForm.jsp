...
<script type="text/javascript">
   	...
   	$(document).ready(function()
   	{
        ...
        // jQuery-UI 캘린터를 불러오는 함수 처리 (datepicker())
        $("#birthday").datepicker(
        {
            dateFormat : "yy-mm-dd"
            ,changeMonth : true
        ...
        $("#submitBtn").click(function()
        {
			...
			//alert($("#minBasicPay").html());	//--(O)
			//alert($("#minBasicPay").text());	//--(O)		→ parseInt 해줘야함.
			
            if($("#minBasicPay").html() > parseInt($("#basicPay").val()))
            {
    ...
    function ajaxRequest()
    {
        ...
        // $.post() / $.get()
        //-- jQeuery 에서 Ajax 를 써야 할 경우 지원해주는 함수.
        //    (서버 측에서 요청한 데이터를 받아오는 기능의 함수)

        // ※ 이 함수($.post())의 사용 방법(방식)
        //--  $.post(오청주소, 전송데이터, 응답액션처리)
        //     '요청주소(url)
        //      → 데이터를 요청할 파일에 대한 정보
        //      '전송데이터(data)
		...
		<form action="employeeupdate.action" method="post" id="employeeForm">
			<table>
				...
				<tr>
					<th>주민번호</th>
					<td>
						<input type="text" id="ssn1" name="ssn1" style="width: 100px;" placeholder="앞 6자리" value="${employee.ssn1 }">
						 - 
				...
				<tr>
					<th>생년월일</th>
					<td><input type="text" id="birthday" name="birthday" placeholder="생년월일" value="${employee.birthday }"></td>
				</tr>
				<tr>
					<th>양/음력</th>
					<td><input type="radio" value="0" name="lunar" id="lunar0" ${employee.lunar==0? "checked=\"checked\"" : "" }>
				...
				<tr>
					<th>전화번호</th>
					<td><input type="tel" id="telephone" name="telephone" placeholder="전화번호" value="${employee.telephone }"></td>
				</tr>
				<tr>
					<th>지역</th>
				...
				<tr>
					...
					<td><select id="departmentId" name="departmentId">
							...
							<c:forEach var="department" items="${departmentList }">
								<option value="${department.departmentId }" ${employee.departmentId == department.departmentId ? "selected=\"selected\"" : "" }>${department.departmentName }</option>
							</c:forEach>
					</select></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><input type="text" id="basicPay" name="basicPay" value="${employee.basicPay }">
						...
						style="color: red; font-weight: bold;">0</span>원)</td>
				</tr>
				<tr>
					<th>수당</th>
					<td><input type="text" id="extraPay" name="extraPay" value="${employee.extraPay }">
					</td>
				</tr>
				<tr>
					<td colspan="2" align="center">
						<br><br>
						<button type="button" class="btn" id="submitBtn" style="width: 40%;">직원 수정</button>
